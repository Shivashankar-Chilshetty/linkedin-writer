...
async function writer(state: typeof State.State) {
    ...
    //another way to invoke system call
...
async function critique(state: typeof State.State) {
    ...
    const response = await model.invoke([
        new SystemMessage(SYSTEM_PROMPT), //system prompt
        lastAIMessage as AIMessage,       //message history
    ]);

    return {
        //trick writer agent into thinking this is a human message
